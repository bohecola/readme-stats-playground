...
export function NumberInput({
  ...
  // Held buttons fire from timers, so read the latestRef value through a ref.
  ...
  const startHold = (direction: 1 | -1) => (e: PointerEvent) => {
    ...
    nudge(direction)
    stopHold()
    // A button disabled mid-holdRef (hit min/max) stops getting pointer events,
    // so also end the holdRef on release anywhere.
    window.addEventListener("pointerup", stopHold, { once: true })
    holdRef.current = setTimeout(() => {
      holdRef.current = setInterval(nudge, HOLD_INTERVAL_MS, direction)
    }, HOLD_DELAY_MS)
  }
    ...
    <div className="flex h-9 items-stretch overflow-hidden rounded-md border border-input shadow-xs transition-colors focus-within:ring-1 focus-within:ring-ring">
      <input
        ...
        onBlur={() => {
          ...
          if (!Number.isNaN(parsed) && clamp(parsed) !== parsed)
