...
pnpm lint       # ESLint + type-check; `pnpm lint:fix` formats
...
```
src/
├── App.tsx                  # page layout and React wiring of the state
├── components/
│   ├── BaseUrlField.tsx     # instance URL display and editor
...
└── lib/
    ...
    ├── cardState.ts         # state model, storage, migration, shared-link overlay (pure, tested)
    ...
    └── *.test.ts            # Vitest unit tests for the helpers above
```
...
Issues and pull requests are welcome. Please make sure `pnpm lint`, `pnpm test` and `pnpm build` pass before submitting (CI checks all three). ESLint doubles as the formatter: run `pnpm lint:fix` instead of formatting by hand.
