...
- **Common style** (theme, colors, border…) is shared across cards in App state (`common`); a card with `ownStyle[card] === true` keeps its own copy instead. The state model, its storage, the old-format migration and the shared-link overlay are pure functions in `src/lib/cardState.ts` (tested); `App.tsx` only wires them to React.
